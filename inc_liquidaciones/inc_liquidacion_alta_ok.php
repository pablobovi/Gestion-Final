<?php
	...
	WHERE idempleado!=1 AND estado=1";
	...
	while ($row_empleados=mysql_fetch_array($q_empleados)) {
        ...
        $conceptos=mysql_query("SELECT * FROM concepto WHERE estado=1");
     	while ($row_conceptos=mysql_fetch_array($conceptos)) {
     		...
     		$totaldetalle=($montofijo+$montovariable)*$cantidad;
